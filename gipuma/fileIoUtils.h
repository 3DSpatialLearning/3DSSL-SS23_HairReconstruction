...
static void readCalibFileKitti(const string calib_filename, Mat_<float> &P1,
                               Mat_<float> &P2) {
    ifstream myfile;
...
static void readBoundingVolume(const string filename, Vec3f &ptBL,
                               Vec3f &ptTR) {
    ifstream myfile;
...
static void readCameraFileStrecha(const string camera_filename,
                                  float &focalLength) {
    // only interested in focal length, but possible to get also other internal
    // and external camera parameters focal length is stored in pixel format as
    // alpha_x and alphy_y, only using alpha_x (which is the very first
    // parameter of the internal camera matrix)
    ifstream myfile;
...
static void readKRtFileMiddlebury(const string filename, vector<Camera> cameras,
    ...
    while (getline(myfile, line)) {
        ...
        string tmp;
        ss >> tmp >> K(0, 0) >> K(0, 1) >> K(0, 2) >> K(1, 0) >> K(1, 1) >>
...
static void readCalibFileDaisy(const string calib_filename, Mat_<float> &P) {
    ...
    while (myfile.getline(line, 512)) {
        if (line[0] == 'p') getProjectionMatrix(line, P);
    }

    myfile.close();
}

static void writeImageToFile(const char *outputFolder, const char *name,
                             const Mat &img) {
    char outputPath[256];
...
static void writeParametersToFile(char *resultsFile, InputFiles inputFiles,
    ...
    myfile << endl;
    if (numPixels != 0) myfile << "Num. pixels: " << numPixels << endl;
    myfile << "\nParameters:" << endl;
    ...
    myfile << "  Kernel size: " << algParameters.box_hsize << " x "
           << algParameters.box_vsize << endl;
    ...
    myfile << "  disparity tolerance (occ check): " << algParameters.dispTol
           << endl;
    myfile << "  normal tolerance (occ check): " << algParameters.normTol
           << endl;
    myfile << "  census epsilon: " << algParameters.census_epsilon << endl;
    myfile << "  self-similarity n: " << algParameters.self_similarity_n
           << endl;
    myfile << "  cost good truncation factor: " << algParameters.good_factor
           << endl;
...
// read ground truth depth map file (dmb) (provided by Tola et al. "DAISY: A
...
static int writeDmbNormal(const char *filename, Mat_<Vec3f> &img) {
    ...
    if (!outimage) printf("Error opening file %s", filename);
...
static int readPfm(const char *filename,
                   ...
                   Mat_<float> &img, long *nx, /*image size in x direction */
                   long *ny)                   /*image size in y direction */
{
    ...
    inimage = fopen(filename, "rb");
    if (!inimage) printf("Error opening file %s", filename);
    ...
    switch (format) /* which format to deal with */
    {
        case '5':   /* P6 format - classic pgm */
            ...
            /* read image data */
            for (j = 0; j < *ny; j++)
                for (i = 0; i < *nx; i++) img(j, i) = (float)getc(inimage);
...
// TODO: convert to float bofore returning
cv::Mat importFloatImage(const std::string &filePath) {
    ...
    try {
        ...
        file.read(reinterpret_cast<char *>(floatImage.ptr()),
                  width * height * sizeof(double));

        // Close the file
        file.close();

        std::cout << "Float image imported successfully: " << filePath
                  << std::endl;
        return floatImage;
