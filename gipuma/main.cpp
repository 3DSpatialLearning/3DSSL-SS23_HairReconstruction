...
static void parse_bundler_3d_points(const char* filename,
    ...
    char line[512];
    if (myfile.peek() == '#') myfile.getline(line, 512);  // skip comment
    myfile.getline(line,
                   512);  //     <num_cameras> <num_points>   [two integers]
    sscanf(line, "%u %u", &num_cameras, &num_points);
    for (size_t i = 0; i < num_cameras; i++) {
        myfile.getline(
            line, 512);  //         <f> <k1> <k2>   [the focal length, followed
                         //         by two radial distortion coeffs]

        myfile.getline(line, 512);  //     <R>             [a 3x3 matrix
                                    //     representing the camera rotation]
        myfile.getline(line, 512);  //     <R>             [a 3x3 matrix
                                    //     representing the camera rotation]
        myfile.getline(line, 512);  //     <R>             [a 3x3 matrix
                                    //     representing the camera rotation]
        myfile.getline(line, 512);  //     <t>             [a 3-vector
    ...
    for (size_t i = 0; i < num_points; i++) {
        Vec3f X;
        myfile.getline(line, 512);  //    <position>      [a 3-vector describing
                                    //    the 3D position of the point]
        sscanf(line, "%f %f %f", &X[0], &X[1], &X[2]);
        myfile.getline(line, 512);  //    <color>         [a 3-vector describing
                                    //    the RGB color of the point]
        myfile.getline(line, 512);  //    <view list>     [a list of views the
                                    //    point is visible in]
        // if (i<10)
...
static int getParametersFromCommandLine(int argc, char** argv,
    ...
    if (inputFiles.pmvs_folder.size() > 0) {
        cout << "Using pmvs information inside directory "
        ...
        cout << "Using image " << inputFiles.img_filenames[camera_idx]
             << " as reference camera" << endl;
        std::swap(inputFiles.img_filenames[0],
                  inputFiles.img_filenames[camera_idx]);
    }
    cout << "Input files are: ";
    for (const auto i : inputFiles.img_filenames) cout << i << " ";
    cout << endl;
...
static void selectCudaDevice() {
    ...
    cout << "Detected gipuma compatible device: " << usableDeviceNames[0]
         << endl;
    ;
...
static int runGipuma(InputFiles& inputFiles, OutputFiles& outputFiles,
    ...
    if (inputFiles.img_filenames.empty()) {
        throw std::runtime_error(
            "There was a problem finding the input files!");
    }
    string ref_name = inputFiles.img_filenames[0].substr(
        0, inputFiles.img_filenames[0].length() - 4);
    sprintf(outputFolder, "%s/%04d%02d%02d_%02d%02d%02d_%s",
            outputFiles.parentFolder, pTime->tm_year + 1900, pTime->tm_mon + 1,
    ...
    if (inputFiles.img_filenames.size() < 2) {
        printf(
            "Command-line parameter error: at least 2 images must be "
            "specified\n");
        return -1;
    }
    ...
    algParams.num_img_processed =
        min((int)numImages, algParams.num_img_processed);
    ...
    CameraParameters cameraParams =
        getCameraParameters(*(gs->cameras), inputFiles, algParams.cam_scale);

    writeParametersToFile(resultsFile, inputFiles, algParams, gtParameters,
                          numPixels);
    ...
    for (int i = 0; i < algParams.num_img_processed; i++) {
        disp[i] =
            Mat::zeros(img_grayscale[0].rows, img_grayscale[0].cols, CV_32F);
        validCost[i] =
            Mat::zeros(img_grayscale[0].rows, img_grayscale[0].cols, CV_8U);
    }

    Mat testImg_display;
    // visualize normals on a halfsphere (just for comparision with normal
    // result image)
    {
        int sizeN = cols / 8;
    ...
    // cout << "Range of Minimum/Maximum depth is: " << algParams.depthMin << "
    // " << algParams.depthMax << endl;
    ...
    cout << "Range of Minimum/Maximum depth is: " << algParams.min_disparity
    ...
    testImg_display.copyTo(
        planes_display(Rect(cols - testImg_display.cols, 0,
    ...
    for (size_t i = 0; i < (size_t)algParams.num_img_processed; i++) {
        ...
        CameraParameters camParamsNotTransformed = getCameraParameters(
    ...
    if (algParams.num_img_processed >= 2) {
        ...
        if (!cameraParams.rectified) {
            convertDisparityDepthImage(disp[0], disp0Temp, cameraParams.f,
                                       cameraParams.cameras[1].baseline);
            convertDisparityDepthImage(disp[1], disp1Temp, cameraParams.f,
                                       cameraParams.cameras[1].baseline);
    ...
    if (algParams.num_img_processed >= 2) {
        ...
        getDisparityForDisplay(disp_occFilling, disp_display, disp_col,
                               algParams.max_disparity);
        writeImageToFile(outputFolder, "dispOccFilling", disp_col);
        disp_display.release();
        disp_col.release();

        getDisparityForDisplay(disp_nocc, disp_nocc_display, disp_nocc_col,
                               algParams.max_disparity);
    ...
    myfile.open(resultsFile, ios::out | ios::app);
    myfile << "\n\nTotal runtime: " << rt << " sec ( " << rt / 60.0f << " min)"
           << endl;
    myfile << "\nRuntime per pixel: " << results.runtime_per_pixel << " ms"
           << endl;
