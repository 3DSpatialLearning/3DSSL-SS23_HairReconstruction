...
// uses smaller (but more) kernels, use if windows watchdog is enabled or if you
// want frequent display updates (might avoid crashes due to timeout on windows
// but possibly slows down marginally) only implements EXTRAPOINTFAR (not
// EXTRAPOINT or EXTRAPOINT2)
#define SMALLKERNEL
 ...
 */
__device__ FORCEINLINE_GIPUMA float disparityDepthConversion_cu(
    const float &f, const float &baseline, const float &d) {
    return f * baseline / d;
}

// CHECKED
__device__ FORCEINLINE_GIPUMA void get3Dpoint_cu(float4 *__restrict__ ptX,
                                                 const Camera_cu &cam,
                                                 const int2 &p,
                                                 const float &depth) {
    ...
    const float4 pt = make_float4(depth * (float)p.x - cam.P_col34.x,
                                  ...
                                  depth - cam.P_col34.z, 0);

    matvecmul4(cam.M_inv, pt, ptX);
}
__device__ FORCEINLINE_GIPUMA void get3Dpoint_cu1(float4 *__restrict__ ptX,
                                                  const Camera_cu &cam,
                                                  const int2 &p) {
    // in case camera matrix is not normalized: see page 162, then depth might
    // not be the real depth but w and depth needs to be computed from that
...
// the camera center
__device__ FORCEINLINE_GIPUMA float getPlaneDistance_cu(const float4 &normal,
                                                        const float4 &X) {
...
// CHECKED
__device__ FORCEINLINE_GIPUMA void getViewVector_cu(float4 *__restrict__ v,
                                                    const Camera_cu &camera,
                                                    const int2 &p) {
...
__device__ FORCEINLINE_GIPUMA static void vecOnHemisphere_cu(
    float4 *__restrict__ v, const float4 &viewVector) {
 ...
 * this method uses the last approach in the link by Muller 1959, Marsaglia
 * 1972: three Gaussian random values for x,y,z Output: random unit vector
 */
// CHECKED
__device__ FORCEINLINE_GIPUMA static void rndUnitVectorSphereMarsaglia_cu(
    float4 *v, curandState *cs) {
    float x = 1.0f;
    float y = 1.0f;
    float sum = 2.0f;
    while (sum >= 1.0f) {
...
// CHECKED
__device__ FORCEINLINE_GIPUMA static void rndUnitVectorOnHemisphere_cu(
    float4 *v, const float4 &viewVector, curandState *cs) {
    rndUnitVectorSphereMarsaglia_cu(v, cs);
    vecOnHemisphere_cu(v, viewVector);
};

__device__ FORCEINLINE_GIPUMA float l1_norm(float f) { return fabsf(f); }
...
__device__ FORCEINLINE_GIPUMA float l1_norm2(float4 f) {
    return (fabsf(f.x) + fabsf(f.y) + fabsf(f.z));
}
template <typename T>
__device__ FORCEINLINE_GIPUMA float weight_cu(const T &c1, const T &c2,
                                              const float &gamma) {
...
// CHECKED
__device__ FORCEINLINE_GIPUMA void getCorrespondingHomographyPt_cu(
    const float *__restrict__ H, int x, int y, float4 *ptf) {
    float4 pt;
...
// CHECKED
__device__ FORCEINLINE_GIPUMA void getCorrespondingPoint_cu(
    const int2 &p, const float *__restrict__ H, float4 *__restrict__ ptf) {
...
__device__ FORCEINLINE_GIPUMA float pmCostComputation_shared(
    const cudaTextureObject_t &l, const T *__restrict__ tile_left,
    const cudaTextureObject_t &r, const T &leftValue, const int2 &pI,
    const float4 &pt_r, const float &tau_color, const float &tau_gradient,
    ...
    /*pt_r.y < rows ) */
    {
        /*float dis = dissimilarity ( l, r, pt_l, pt_r, gradX1, gradY1, gradX2,
         * gradY2, alpha, tau_color, tau_gradient );*/

        /*float colDiff = colorDifferenceL1_cu ( texatpt4(l,pt_l),
         * texatpt4(r,pt_r) );*/

        /*if (*/
        /*pt_li.x == 100 && */
        /*pt_li.y == 100)*/
        /*printf ("PMCOSTCOMPUTATION I and J are %d %d and II JJ are %d %d value
         * is %f and tile cache is %f\n", I, J, II, JJ, leftValue,
         * tile_left[I][J]);*/

        const T gx2 = tex2D<T>(r, pt_r.x + 1 + 0.5f, pt_r.y + 0.5f) -
                      tex2D<T>(r, pt_r.x - 1 + 0.5f, pt_r.y + 0.5f);
        const T gy2 = tex2D<T>(r, pt_r.x + 0.5f, pt_r.y + 1 + 0.5f) -
                      tex2D<T>(r, pt_r.x + 0.5f, pt_r.y - 1 + 0.5f);
        const float colDiff =
            l1_norm(leftValue - tex2D<T>(r, pt_r.x + 0.5f, pt_r.y + 0.5f));
        ...
        // gradient dissimilarity (L1) in x and y direction (multiplication by
        // 0.5 to use tauGrad from PatchMatch stereo paper)
        const float gradDis =
            fminf((l1_norm(gradX) + l1_norm(gradY)) * 0.0625f, tau_gradient);
...
__device__ FORCEINLINE_GIPUMA float pmCostComputation(
    const cudaTextureObject_t &l, const T *__restrict__ tile_left,
    const cudaTextureObject_t &r, const float4 &pt_l, const float4 &pt_r,
    const int &rows, const int &cols, const float &tau_color,
    const float &tau_gradient, const float &alpha, const float &w) {
    /*XXX*/
    /*if ( pt_r.x >= 0 && */
    /*pt_r.x < cols && */
    /*pt_r.y >= 0 && */
    /*pt_r.y < rows ) */
    {
        ...
        const float colDiff =
            l1_norm(tex2D<T>(l, pt_l.x + 0.5f, pt_l.y + 0.5f) -
                    tex2D<T>(r, pt_r.x + 0.5f, pt_r.y + 0.5f));
        const float colDis = fminf(colDiff, tau_color);

        const T gx1 = tex2D<T>(l, pt_l.x + 1 + 0.5f, pt_l.y + 0.5f) -
                      tex2D<T>(l, pt_l.x - 1 + 0.5f, pt_l.y + 0.5f);
        const T gy1 = tex2D<T>(l, pt_l.x + 0.5f, pt_l.y + 1 + 0.5f) -
                      tex2D<T>(l, pt_l.x + 0.5f, pt_l.y - 1 + 0.5f);
        const T gx2 = tex2D<T>(r, pt_r.x + 1 + 0.5f, pt_r.y + 0.5f) -
                      tex2D<T>(r, pt_r.x - 1 + 0.5f, pt_r.y + 0.5f);
        const T gy2 = tex2D<T>(r, pt_r.x + 0.5f, pt_r.y + 1 + 0.5f) -
                      tex2D<T>(r, pt_r.x + 0.5f, pt_r.y - 1 + 0.5f);
...
__device__ FORCEINLINE_GIPUMA void getHomography_real(
    const float *K1_inv, const float *K2, const float *R, const float4 t,
    const float4 n, const float d, float *H) {
    /*print_matrix(R,"R");*/
    float tmp[16];
    float tmp2[16];
...
__device__ FORCEINLINE_GIPUMA void getHomography_cu(
    const Camera_cu &from, const Camera_cu &to,
...
/* census transform, get value based on weather intensity is smaller or higher
 * than center intensity Input: p        - intensity of center pixel pNb      -
 * intensity of current nb pixel in the kernel epsilon  - threshold for
 * classifying as the same intensity (for original ct no epsilon is used -->
 * epsilon=0)
 */
__device__ FORCEINLINE_GIPUMA uint8_t getCTbit_cu(float p, float pNb,
                                                  float eps) {
    uint8_t bit = 1;
    if (p - pNb > eps)
        bit = 0;
...
__device__ FORCEINLINE_GIPUMA float ct_Arma_cu(
    const cudaTextureObject_t &l, const cudaTextureObject_t &r, const int2 &p,
    const int vRad, const int hRad, const float intensityCenterLeft,
    const float intensityCenterRight, const float eps,
    ...
    // pt ( 1 ) < ( float ) ( l.rows - 1 ) )
    {
        ...
        if (getCTbit_cu(intensityCenterLeft, intensityL, eps) ==
            getCTbit_cu(intensityCenterRight, intensityR, eps))
            c = 0.0f;
        else
            c = 1.0f;
    }
 ...
 * check if intensity of neighboring pixel is lower or higher than intensity of
 * center pixel
 */
__device__ float censusTransform_Arma_cu(const cudaTextureObject_t &l,
                                         const cudaTextureObject_t &r,
                                         const int2 &p, const float &d,
                                         const int &vRad, const int &hRad,
                                         const float &eps,
                                         const float *__restrict__ H) {
    float cost = 0.0f;
    float4 pt;
    ...
    for (int i = p.x - hRad; i <= p.x + hRad; i++) {
        for (int j = p.y - vRad; j <= p.y + vRad; j++) {
            ...
            float c =
                ct_Arma_cu(l, r, make_int2(i, j), vRad, hRad,
                           intensityCenterLeft, intensityCenterRight, eps, H);
...
__device__ FORCEINLINE_GIPUMA static float pmCost(
    const cudaTextureObject_t &l, const T *__restrict__ tile_left,
    const int2 tile_offset, const cudaTextureObject_t &r, const int &x,
    const int &y, const float4 &normal, const int &vRad, const int &hRad,
    const AlgorithmParameters &algParam, const CameraParameters_cu &camParams,
    const int &camTo) {
    ...
    float H[16];
    /*float H[3*3];*/
    getHomography_cu(camParams.cameras[REFERENCE], camParams.cameras[camTo],
                     camParams.cameras[REFERENCE].K_inv,
                     camParams.cameras[camTo].K, normal, normal.w, H);
...

    {
        ...
        for (int i = -hRad; i < hRad + 1; i += WIN_INCREMENT) {
            for (int j = -vRad; j < vRad + 1; j += WIN_INCREMENT) {
                ...
                float4 pt;
                getCorrespondingPoint_cu(make_int2(xTemp, yTemp), H, &pt);

                cost = cost + pmCostComputation<T>(l, tile_left, r, pt_l, pt,
                                                   rows, cols, tau_color,
                                                   tau_gradient, alpha, w);
...
__device__ FORCEINLINE_GIPUMA static float hasImageTexture(
    const cudaTextureObject_t &l, const int2 &p, const int &vRad,
    const int &hRad, const AlgorithmParameters &algParam) {
    const float gamma = algParam.gamma;
    ...
    for (int i = -hRad; i < hRad + 1; i += WIN_INCREMENT) {
        for (int j = -vRad; j < vRad + 1; j += WIN_INCREMENT) {
            ...
            const float w =
                weight_cu<T>(tex2D<T>(l, pt_l.x + 0.5f, pt_l.y + 0.5f),
                             tex2D<T>(l, p.x + 0.5f, p.y + 0.5f), gamma);
            if (w > algParam.no_texture_sim) count_similar_pixel++;
        }
    }
    if (count_similar_pixel > hRad * vRad * 4 /
                                  (WIN_INCREMENT * WIN_INCREMENT) *
                                  algParam.no_texture_per)
        return false;
    return true;
}
...
__device__ FORCEINLINE_GIPUMA static float hasImageTexture_shared(
    const cudaTextureObject_t &l, const T *__restrict__ tile_left,
    const int2 tile_offset, const int2 &p, const int &vRad, const int &hRad,
    const AlgorithmParameters &algParam) {
    const T centerValue =
        tile_left[p.x - tile_offset.x + SHARED_SIZE_W * (p.y - tile_offset.y)];

    int count_similar_pixel = 0;
    for (int i = -hRad; i < hRad + 1; i += WIN_INCREMENT) {
        for (int j = -vRad; j < vRad + 1; j += WIN_INCREMENT) {
            const int2 pI =
                make_int2(p.x + i - tile_offset.x, p.y + j - tile_offset.y);
            const T leftValue = tile_left[pI.x + SHARED_SIZE_W * pI.y];
            const float w =
                weight_cu<T>(leftValue, centerValue, algParam.gamma);
            // if (p.x == 440 && p.y == 307 )
    ...
    ////printf("Limit is %f\n", (float) 4*hRad*vRad/(WIN_INCREMENT *
    ///WIN_INCREMENT)*algParam.no_texture_per); /printf("Hrad is %d\n", vRad);
    //}
    if (count_similar_pixel > hRad * vRad * 4 /
                                  (WIN_INCREMENT * WIN_INCREMENT) *
                                  algParam.no_texture_per)
...
__device__ FORCEINLINE_GIPUMA static float pmCost_shared(
    const cudaTextureObject_t &l, const T *__restrict__ tile_left,
    const int2 tile_offset, const cudaTextureObject_t &r, const int2 &p,
    const float4 &normal, const int &vRad, const int &hRad,
    const AlgorithmParameters &algParam, const CameraParameters_cu &camParams,
    const int &camTo) {
    const float alpha = algParam.alpha;
    const float tau_color = algParam.tau_color;
    ...
    // camParams.cameras[camTo], camParams.K_inv, camParams.K, normal, normal.w,
    // H );
    getHomography_cu(camParams.cameras[REFERENCE], camParams.cameras[camTo],
                     camParams.cameras[REFERENCE].K_inv,
                     camParams.cameras[camTo].K, normal, normal.w, H);
...

    {
        ...
        const T centerValue = tile_left[p.x - tile_offset.x +
                                        SHARED_SIZE_W * (p.y - tile_offset.y)];

#ifdef CENSUS
        cost = censusTransform_Arma_cu(l, r, p, normal.w, vRad, hRad,
                                       algParam.census_epsilon, H);
        return cost;
#endif
        ...
        for (int i = -hRad; i < hRad + 1; i += WIN_INCREMENT) {
            for (int j = -vRad; j < vRad + 1; j += WIN_INCREMENT) {
                 ...
                 * J,leftValue);*/
                /*}*/
                w = weight_cu<T>(leftValue, centerValue, gamma);
                // if( p.x == 446 && p.y == 307)
                // printf("weigth is %f\n", w);

                // const float w = weight_cu ( tile_left[xTemp -
                // SHARED_SIZE_H][], pt_l), texat(l,x,y), gamma);

                float4 pt;
                getCorrespondingPoint_cu(pTemp, H, &pt);

                cost = cost + pmCostComputation_shared<T>(
                                  l, tile_left, r, leftValue, pI, pt, tau_color,
                                  tau_gradient, alpha, w);
                // weightSum = weightSum + w;
            }
        }
...
__device__ FORCEINLINE_GIPUMA float getDepthFromPlane3_cu(const Camera_cu &cam,
    ...
    return -d * cam.fx /
           ((n.x * (p.x - cam.K[2])) +
            (n.y * (p.y - cam.K[2 + 3])) * cam.alpha + n.z * cam.fx);
}
...
__device__ FORCEINLINE_GIPUMA static float pmCostMultiview_cu(
    const cudaTextureObject_t *images, const T *__restrict__ tile_left,
    const int2 tile_offset, const int2 p, const float4 &normal, const int &vRad,
    const int &hRad, const AlgorithmParameters &algParam,
    const CameraParameters_cu &camParams, const float4 *__restrict__ state,
    const int point) {
    ...
    for (int i = 0; i < camParams.viewSelectionSubsetNumber; i++) {
        ...
        /*if ( idxCurr != REFERENCE ) */
        {
...
#ifdef SHARED
            if (tile_offset.x != 0)
                c = pmCost_shared<T>(images[REFERENCE], tile_left, tile_offset,
                                     images[idxCurr], p, normal, vRad, hRad,
                                     algParam, camParams, idxCurr);
            else
#endif
                c = pmCost<T>(images[REFERENCE], tile_left, tile_offset,
                              images[idxCurr], p.x, p.y, normal, vRad, hRad,
                              algParam, camParams, idxCurr);
            ...
            if (c < MAXCOST)
                ...
                c = MAXCOST;  // in order to not get an overflow when
                              // accumulating
            costVector[i] = c;
            cost_count++;
        }
    ...
    // for some robustness only consider best n cost values (n dependent on
    // number of images)
    ...
    if (numConsidered < 1) cost = MAXCOST;

    if (cost != cost || cost > MAXCOST || cost < 0) cost = MAXCOST;

    return cost;
}

__device__ FORCEINLINE_GIPUMA float get_smoothness_at2(
    const float4 *__restrict__ state, const float4 &norm, const float &depth,
    const int2 p, const int2 p_other, const int cols, const Camera_cu &cam) {
    float4 norm_other = state[p_other.x + p_other.y * cols];
    const float depth_other =
        getDisparity_cu(norm_other, norm_other.w, p_other, cam);
...
#define ISDISPDEPTHWITHINBORDERS(disp, camParams, camIdx, algParams) \
    disp >= camParams.cameras[REFERENCE].depthMin &&disp <=          \
        camParams.cameras[REFERENCE].depthMax

template <typename T>
__device__ FORCEINLINE_GIPUMA void spatialPropagation_cu(
    const cudaTextureObject_t *imgs, const T *__restrict__ tile_left,
    const int2 &tile_offset, const int2 &p, const int &box_hrad,
    const int &box_vrad, const AlgorithmParameters &algParams,
    const CameraParameters_cu &camParams, float *cost_now, float4 *norm_now,
    const float4 norm_before, float *disp_now, const float4 *__restrict__ state,
    const int point) {
    // previous image values

    const float d_before = norm_before.w;
    const float disp_before =
        getDisparity_cu(norm_before, d_before, p, camParams.cameras[REFERENCE]);

    float cost_before = pmCostMultiview_cu<T>(
        imgs, tile_left, tile_offset, p, norm_before, box_vrad, box_hrad,
        algParams, camParams, state, point);

    if (ISDISPDEPTHWITHINBORDERS(disp_before, camParams, REFERENCE,
                                 algParams)) {
...
/* compute random disparity and unit vector within given intervals, used for
 * plane refinement step interval is limited by image border and general
 * disparity range [0 maxDisparity] Input: x     - current column x disp  - old
 * disparity value norm  - old normal maxDeltaZ  - range radius for disparity
 * [disp-maxDeltaZ,disp+maxDeltaZ] maxDeltaN  - range radius for normal
 *        maxDisparity  - maximum disparity value
 *        cols - number of columns of the image
 *        dir - disparity to the left or right of x
 *        limit - defines maximal value for |[nx ny]T| so that only plane tilts
 * to a certain degree are possible Output: dispOut - new disparity normOut -
 * new normal
 */
__device__ FORCEINLINE_GIPUMA void getRndDispAndUnitVector_cu(
    float disp, const float4 norm, float &dispOut, float4 *__restrict__ normOut,
    const float maxDeltaZ, const float maxDeltaN, const float minDisparity,
    const float maxDisparity, curandState *cs, CameraParameters_cu &camParams,
    const float baseline, const float4 viewVector) {
    ...
    maxDelta =
        min(maxDeltaZ, maxDisparity - disp);  // limit new disp < maxDisparity
...
__device__ FORCEINLINE_GIPUMA static void planeRefinement_cu(
    const cudaTextureObject_t *images, const T *__restrict__ tile_left,
    const int2 &p, const int2 &tile_offset, const int &box_hrad,
    const int &box_vrad, const AlgorithmParameters &algParams,
    CameraParameters_cu &camParams, const int camIdx,
    float *__restrict__ cost_now, float4 *__restrict__ norm_now,
    float *__restrict__ disp_now, curandState *cs,
    ...
    // divide delta by 4 instead of 2 for less iterations (for higher disparity
    // range) iteration is done over disparity values even for multi-view case
    // in order to have approximately unifom sampling along epipolar line
    /*for ( float deltaZ = ( float ) algParams.max_disparity / 2.0f; deltaZ >=
     * 0.1f; deltaZ = deltaZ / 4.0f ) {*/
    float4 norm_temp;
    float dispTemp_L;
    float dTemp_L;
    float costTempL;
    ...
    for (float deltaZ = maxdisp; deltaZ >= 0.01f; deltaZ = deltaZ / 10.0f) {
        getRndDispAndUnitVector_cu(*disp_now, *norm_now, dispTemp_L, &norm_temp,
                                   deltaZ, deltaN, algParams.min_disparity,
                                   algParams.max_disparity, cs, camParams,
                                   camParams.cameras[0].baseline, viewVector);

        dTemp_L = getD_cu(norm_temp, p, dispTemp_L, camParams.cameras[camIdx]);

        norm_temp.w = dTemp_L;  // TODO might save a variable here
        costTempL = pmCostMultiview_cu<T>(images, tile_left, tile_offset, p,
                                          norm_temp, box_vrad, box_hrad,
                                          algParams, camParams, state, 0);
...
__global__ void gipuma_init_cu2(GlobalState &gs) {
    ...
    gs.lines->c[center] = pmCostMultiview_cu<T>(
        gs.imgs, tile_leftt, tmp, p, norm_now, box_vrad, box_hrad, *(gs.params),
        *(gs.cameras), gs.lines->norm4, 0);
    return;
}
template <typename T>
__global__ void gipuma_initial_cost(GlobalState &gs) {
    ...
    const int2 tmp = make_int2(0, 0);
    gs.lines->c[center] = pmCostMultiview_cu<T>(
        gs.imgs, tile_leftt, tmp, p, gs.lines->norm4[center], box_vrad,
        box_hrad, *(gs.params), *(gs.cameras), gs.lines->norm4, 0);
...
__global__ void gipuma_compute_disp(GlobalState &gs) {
    ...
    matvecmul4(gs.cameras->cameras[REFERENCE].R_orig_inv, norm,
               (&norm_transformed));
...
template <typename T>
__device__ FORCEINLINE_GIPUMA void gipuma_checkerboard_cu(
    GlobalState &gs, int2 p, const int2 tile_offset, int iter) {
    ...
    {
        int ii =
            7 * (threadIdx.y * TILE_W + threadIdx.x);  // flattened indexing
        ...
        for (int t = 0; t < 8; t++) {
            ...
            if (ii < SHARED_SIZE)
                tile_left[ii++] =
                    tex2D<T>(imgs[REFERENCE], x1 + 0.5f, y1 + 0.5f);
        }
    }
    ...
    disp_now =
        getDisparity_cu(norm_now, norm_now.w, p, camParams.cameras[REFERENCE]);
    // norm_now.z = -sqrtf(1.0f - (norm_now.x*norm_now.x +
    // norm_now.y*norm_now.y));
...
#ifdef SHARED
        if (tile_offset.x != 0)
            hasTexture = hasImageTexture_shared<T>(imgs[REFERENCE], tile_left,
                                                   tile_offset, p, box_vrad,
                                                   box_hrad, algParams);
        else
#endif
            hasTexture = hasImageTexture<T>(imgs[REFERENCE], p, box_vrad,
                                            box_hrad, algParams);
        if (!hasTexture) {
            c[center] = MAXCOST;
            return;
...
#define SPATIALPROPAGATION(point)                                       \
    spatialPropagation_cu<T>(imgs, tile_left, tile_offset, p, box_hrad, \
                             box_vrad, algParams, camParams, &cost_now, \
                             &norm_now, norm[point], &disp_now, norm, point)
    ...
    if (p.x < cols - 1 && p.y < rows - 2) {
        SPATIALPROPAGATION(right + cols * 2);
    }
#endif

    planeRefinement_cu<T>(imgs, tile_left, p, tile_offset, box_hrad, box_vrad,
                          algParams, camParams, REFERENCE, &cost_now, &norm_now,
                          &disp_now, &localState, norm);
...
template <typename T>
__device__ FORCEINLINE_GIPUMA void gipuma_checkerboard_spatialPropFar_cu(
    GlobalState &gs, int2 p, const int2 tile_offset, int iter) {
    int box_hrad = (gs.params->box_hsize - 1) / 2;
    int box_vrad = (gs.params->box_vsize - 1) / 2;
...
template <typename T>
__device__ FORCEINLINE_GIPUMA void gipuma_checkerboard_spatialPropClose_cu(
    GlobalState &gs, int2 p, const int2 tile_offset, int iter) {
    int box_hrad = (gs.params->box_hsize - 1) / 2;
    int box_vrad = (gs.params->box_vsize - 1) / 2;
...
template <typename T>
__device__ FORCEINLINE_GIPUMA void gipuma_checkerboard_planeRefinement_cu(
    GlobalState &gs, int2 p, const int2 tile_offset, int iter) {
    int box_hrad = (gs.params->box_hsize - 1) / 2;
    int box_vrad = (gs.params->box_vsize - 1) / 2;
...
void gipuma(GlobalState &gs) {
    ...
    // printf("Launching kernel with grid of size %d %d and block of size %d %d
    ...
    // grid_size_initrand.x, grid_size_initrand.y, block_size_initrand.x,
    // block_size_initrand.y);
    ...
    for (int it = 0; it < maxiter; it++) {
        ...
        gipuma_black_spatialPropClose_cu<T>
            <<<grid_size, block_size, shared_size_host * sizeof(T)>>>(gs, it);
