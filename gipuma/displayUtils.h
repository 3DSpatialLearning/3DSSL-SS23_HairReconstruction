...
/* compute gamma correction (just for display purposes to see more details in
 * farther away areas of disparity image) Input: img   - image gamma - gamma
 * value Output: gamma corrected image
 */
...
static void getDisparityForDisplay(const Mat_<float> &disp, Mat &dispGray,
                                   Mat &dispColor, float numDisparities,
                                   float minDisp = 0.0f) {
    float gamma = 2.0f;  // to get higher contrast for lower disparity range
                         // (just for color visualization)
    disp.convertTo(dispGray, CV_16U, 65535.f / (numDisparities - minDisp),
                   -minDisp * 65535.f / (numDisparities - minDisp));
    Mat disp8;
    disp.convertTo(disp8, CV_8U, 255.f / (numDisparities - minDisp),
                   -minDisp * 255.f / (numDisparities - minDisp));
    ...
    for (int y = 0; y < dispColor.rows; y++) {
        for (int x = 0; x < dispColor.cols; x++) {
            if (disp(y, x) <= 0.0f) dispColor.at<Vec3b>(y, x) = Vec3b(0, 0, 0);
        }
    }
}

static void convertDisparityDepthImage(const Mat_<float> &dispL, Mat_<float> &d,
...
static string getColorString(Vec3i color) {
    stringstream ss;
    ss << (int)((float)color(2) / 256.f) << " "
       << (int)((float)color(1) / 256.f) << " "
       << (int)((float)color(0) / 256.f);
    return ss.str();
}
...
template <typename ImgType>
static void storePlyFileBinary(char *plyFilePath, const Mat_<float> &depthImg,
                               const Mat_<Vec3f> &normals,
                               const Mat_<ImgType> img, Camera cam,
                               Mat_<float> &distImg) {
    ...
    for (int x = 0; x < depthImg.cols; x++) {
        for (int y = 0; y < depthImg.rows; y++) {
            ...
            */
            ...
            // Vec3f ptX_v1 =
            // get3dPointFromPlane(cam.P_inv,cam.C,n,planes.d(y,x),x,y); cout <<
            // ptX_v1 << " / " << ptX << endl;
...
#pragma omp critical
            {
                // myfile << ptX(0) << " " << ptX(1) << " " << ptX(2) << " " <<
                // n(0) << " " << n(1) << " " << n(2) << " " <<
                // getColorString(color) << endl;
                fwrite(&(ptX(0)), sizeof(float), 3, outputPly);
            ...
            distImg(y, x) =
                sqrt(pow(ptX(0) - cam.C(0), 2) + pow(ptX(1) - cam.C(1), 2) +
                     pow(ptX(2) - cam.C(2), 2));
...
template <typename ImgType>
static void storePlyFile(char *plyFilePath, const Mat_<float> &depthImg,
                         const Mat_<Vec3f> &normals, const Mat_<ImgType> img,
                         Camera cam, Mat_<float> &distImg) {
...
static void getNormalsForDisplay(const Mat &normals, Mat &normals_display,
                                 int rtype = CV_16U) {
    if (rtype == CV_8U)
