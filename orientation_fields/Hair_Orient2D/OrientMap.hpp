...
public:
    ...
    void diffuse(const string &filename, float lowConfThresh = 0.01f, int numIters = 40)
    {
        ...
        const vec2 &point, vec2 &orient, const vec2 *refOrient = NULL) const
    {
        ...
        if (refOrient && (orient DOT * refOrient) < 0)
            orient = -orient;
        return conf;
    }
        ...
        const vec2 &point2, float mag2) const
    {
        ...
        return (f < 1.0f ? point0 * (1.0f - f) + point1 * f : point1 * (2.0f - f) + point2 * (f - 1.0f));
    }
