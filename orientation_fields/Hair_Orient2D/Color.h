...
namespace trimesh
{
	...
	class Color : public CVec<3, float>
	{
	public:
		Color()
		{
		}
		Color(const CVec<3, float> &v_) : CVec<3, float>(v_)
		{
		}
		Color(const CVec<3, double> &v_) : CVec<3, float>((float)v_[0], (float)v_[1], (float)v_[2])
		{
		}
		Color(float r, float g, float b) : CVec<3, float>(r, g, b)
		{
		}
		Color(double r, double g, double b) : CVec<3, float>((float)r, (float)g, (float)b)
		{
		}
		explicit Color(const float *rgb) : CVec<3, float>(rgb[0], rgb[1], rgb[2])
		{
		}
		explicit Color(const double *rgb) : CVec<3, float>((float)rgb[0], (float)rgb[1], (float)rgb[2])
		{
		}

		// Implicit conversion from float would be bad, so we have an
		// explicit constructor and an assignment statement.
		explicit Color(float c) : CVec<3, float>(c, c, c)
		{
		}
		explicit Color(double c) : CVec<3, float>((float)c, (float)c, (float)c)
		{
		}
	...
	private:
		...
		{
			return Color(v[0], v[1] - 0.5f, v[2] - 0.5f).col_transform(1.0f, 0.0f, 1.402f, 1.0f, -0.34413629f, -0.71413629f, 1.0f, 1.772f, 0.0f);
		}
